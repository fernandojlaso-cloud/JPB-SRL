...
import { GymDataLogo } from './components/GymDataLogo';
...
function MainDashboard() {
  ...
  const handleExportExcel = () => {
    ...
    const fileName = selectedProjectId === 'macro' 
      ? 'Grupo_Simetris_Control_Financiero_Macro.xlsx'
      : `Grupo_Simetris_${project ? project.name.replace(/\s+/g, '_') : 'Obra'}.xlsx`;
  ...
  const handleResetData = () => {
    if (confirm('¿Deseas restaurar los datos de ejemplo iniciales del archivo Excel (Casa Mily y Fer Caracas 2672)?')) {
