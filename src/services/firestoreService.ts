...
export async function restoreCompleteBackup(backup: any): Promise<{ success: boolean; count: number }> {
  if (!backup || (!backup.projects && !backup.transactions)) {
  ...
  if (Array.isArray(backup.projects)) {
    for (const p of backup.projects) {
      if (p.id) {
        await setDoc(doc(db, COLLECTIONS.PROJECTS, p.id), p, { merge: true });
        totalItems++;
      }
    }
  }
  ...
  if (Array.isArray(backup.budgets)) {
    for (const b of backup.budgets) {
      if (b.id) {
        await setDoc(doc(db, COLLECTIONS.BUDGETS, b.id), b, { merge: true });
        totalItems++;
      }
    }
  }
  ...
  if (Array.isArray(backup.transactions)) {
    ...
    for (let i = 0; i < backup.transactions.length; i += chunkSize) {
      ...
      chunk.forEach((t: any) => {
        if (t.id) {
          batch.set(doc(db, COLLECTIONS.TRANSACTIONS, t.id), t, { merge: true });
          totalItems++;
        }
      });
      await batch.commit();
